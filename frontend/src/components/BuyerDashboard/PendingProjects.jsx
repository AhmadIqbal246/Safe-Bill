  ...
  const handleProjectAction = async (projectId, action) => {
    ...
    if (action === 'approve') {
      if (project && project.invite_token) {
        navigate(`/project-invite?token=${project.invite_token}`);
      }
      return;
    }
          ...
          {pendingProjects.map((project) => (
            <div key={project.id} className="border border-gray-100 rounded-lg p-4 hover:bg-gray-50 transition-colors">
              ...
              <div className="flex items-start justify-between mb-3">
                <div className="flex-1">
                  ...
                  <div className="flex items-center gap-4 text-sm text-gray-500">
                    <span>{t('buyer_dashboard.seller')}: {project.seller_name}</span>
                    <span>{t('buyer_dashboard.ref')}: {project.reference_number}</span>
                    <span>{t('buyer_dashboard.created')}: {new Date(project.created_at).toLocaleDateString()}</span>
                  </div>
                </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                <div>
                  <div className="text-xs text-gray-400 font-medium mb-1">{t('buyer_dashboard.total_amount')}</div>
                ...
                <div>
                  <div className="text-xs text-gray-400 font-medium mb-1">{t('buyer_dashboard.installments_label')}</div>
                  <div className="text-sm text-gray-700">
                    {t('buyer_dashboard.payments_count', { count: project.installments?.length || 0 })}
                  </div>
                </div>
              </div>
              ...
              {actionMessage[project.id] && (
                <div className={`mb-4 p-3 rounded-lg text-sm ${actionMessage[project.id].includes('approved') ? 'bg-green-100 text-green-800' :
              ...
              <div className="flex flex-col sm:flex-row gap-3">
                <button
                  ...
                  className="flex-1 bg-green-600 text-white py-2 px-4 rounded-lg font-medium hover:bg-green-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer flex items-center justify-center gap-2"
                >
                  {actionLoading[project.id] ? (
                    ...
                    <>
                      ...
                      {t('buyer_dashboard.review_and_pay') || 'Review & Pay'}
                    </>
                  )}
                </button>
                ...
                <button
                  ...
                  disabled={actionLoading[project.id]}
                  className="flex-1 bg-red-600 text-white py-2 px-4 rounded-lg font-medium hover:bg-red-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer flex items-center justify-center gap-2"
                >
                  {actionLoading[project.id] ? (
                    <>
                      <div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" />
