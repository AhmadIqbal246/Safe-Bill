...
export default function DashboardSummary({ stats = {} }) {
  ...
  return (
    <>
      ...
      <div style={{ flex: 1, background: '#fff', borderRadius: 12, boxShadow: '0 1px 4px #e5e7eb', padding: 24, minWidth: 220 }}>
        <div style={{ fontSize: 18, fontWeight: 500 }}>{t('billings.payments_held_for_projects')}</div>
        <div style={{ fontSize: 32, fontWeight: 700, color: '#153A7D' }}>
          {parseFloat(paymentsHeldForProjects).toLocaleString()} {currency}
        </div>
        {/* <div style={{ color: '#6b7280', fontSize: 14 }}>{t('buyer_dashboard.held_in_escrow')}</div> */}
      </div>
    </>
  );
}